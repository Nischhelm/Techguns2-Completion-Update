...
public abstract class GenericNPCGearSpecificStats extends GenericNPC {
    ...
    public float getTotalArmorAgainstType(TGDamageSource dmgsrc) {
        ...
        for (ItemStack stack : inv) {
            if (!stack.isEmpty() && stack.getItem() instanceof ItemArmor armor) {
                ...
                if (stack.getItem() instanceof GenericArmor genArmor) {
                    float armorvalue = genArmor.getArmorValue(dmgsrc.damageType);
    ...
    public float getPenetrationResistance(TGDamageSource dmgsrc) {
        ...
        float res = 0.0f;
        for (ItemStack stack : inv) {
            if (!stack.isEmpty() && stack.getItem() instanceof GenericArmor genArmor) {
                float antipen = genArmor.getToughness();
                res += antipen;
            }
        }
        return res;
    ...
    public float getToughnessAfterPentration(EntityLivingBase elb, TGDamageSource src) {
        ...
        for (ItemStack stack : inv) {
            if (!stack.isEmpty() && stack.getItem() instanceof GenericArmor genArmor) {
                float dt = genArmor.toughness - (Math.max(src.armorPenetration - genArmor.getToughness(), 0f));
                t += Math.max(dt, 0f);
            } else if (!stack.isEmpty() && stack.getItem() instanceof ItemArmor a) {
