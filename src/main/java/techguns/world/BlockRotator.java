...
public class BlockRotator {
    ...
    public static IBlockState getRotatedHorizontal(IBlockState state, int times) {
        ...
        if (state.getProperties().containsKey(BlockHorizontal.FACING)) {
            ...
            return rotateBooleanPlane(state, times);
        } else if (state.getProperties().containsKey(BlockLog.LOG_AXIS)) {
            if (times == 1 || times == 3) {
                if (state.getValue(BlockLog.LOG_AXIS) == EnumAxis.X)
                    return state.withProperty(BlockLog.LOG_AXIS, EnumAxis.Z);
                if (state.getValue(BlockLog.LOG_AXIS) == EnumAxis.Z)
    ...
    protected static IBlockState rotateBooleanPlane(IBlockState state, int times) {
        boolean plane = state.getValue(BlockTGDoor3x3.ZPLANE);
        if ((times % 2) == 0) {
            return state;
        } else {
            return state.withProperty(BlockTGDoor3x3.ZPLANE, !plane);
        }
    }
